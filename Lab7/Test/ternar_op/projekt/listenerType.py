...
class ListenerType(PLC_Lab8_exprListener):
    ...
    def enterTernar(self, ctx):
        pass

    # Exit a parse tree produced by PLC_Lab8_exprParser#ternar.
    def exitTernar(self, ctx):
        cond_t = self.types[ctx.expr(0)]
        t1 = self.types[ctx.expr(1)]
        t2 = self.types[ctx.expr(2)]
        ...
        if t1 in ("int", "float") and t2 in ("int", "float"):
            self.types[ctx] = "float" if "float" in (t1, t2) else "int"
        ...
        # bool
            ...
            raise TypeError("Incompatible types in ternary operator")
